...
// The function records each object's start offset and writes a correct
// traditional xref + trailer + startxref + EOF marker. Returns the bytes.
//
// extraTrailerKeys, if non-empty, is inlined into the trailer dict (e.g.,
// `/Prev 47` for cycle-detection tests). Only the inline portion is supplied;
// the helper handles `/Size` and `/Root`.
